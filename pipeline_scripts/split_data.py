...
def remove_stor_prefix(municipality: str) -> str:
    if isinstance(municipality, str) and municipality.lower().startswith("stor-"):
        return municipality[5:].strip()
    return municipality.strip() if isinstance(municipality, str) else municipality
...
def main():
    ...
    df["Municipality"] = df["Municipality"].apply(remove_stor_prefix)
